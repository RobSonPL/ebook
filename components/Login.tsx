...
import { BookOpen, Sparkles, Mail, Lock } from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const performLogin = async (emailToLogin: string) => {
    setIsLoading(true);
    try {
      const user = await mockLogin(emailToLogin);
      onLoginSuccess(user);
    } catch (err) {
      console.error(err);
